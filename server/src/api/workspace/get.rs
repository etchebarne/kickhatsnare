...
use kickhatsnare_protocol::{ErrorCode, workspace::GetWorkspaceParams};
use serde_json::Value;

use super::{ApiError, core_error, serialize_snapshot};

pub(super) fn handle(params: &Value, workspaces: &Workspaces) -> Result<Value, ApiError> {
    serde_json::from_value::<GetWorkspaceParams>(params.clone())
