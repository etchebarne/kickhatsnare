...
use kickhatsnare_protocol::{ErrorCode, workspace::MoveWorkspaceEntryParams};
...
pub(super) fn handle(params: &Value, workspaces: &mut Workspaces) -> Result<Value, ApiError> {
    let params = serde_json::from_value::<MoveWorkspaceEntryParams>(params.clone())
        .map_err(|error| ApiError::new(ErrorCode::InvalidParams, error.to_string()))?;
    workspaces
        .move_entry(params.source_path, params.destination_path)
