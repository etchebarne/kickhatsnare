...
use kickhatsnare_protocol::{ErrorCode, workspace::CreateWorkspaceDirectoryParams};
...
pub(super) fn handle(params: &Value, workspaces: &mut Workspaces) -> Result<Value, ApiError> {
    let params = serde_json::from_value::<CreateWorkspaceDirectoryParams>(params.clone())
