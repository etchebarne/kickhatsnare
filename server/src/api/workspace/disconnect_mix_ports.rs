...
use kickhatsnare_protocol::{ErrorCode, workspace::DisconnectMixPortsParams};
...
pub(super) fn handle(params: &Value, workspaces: &mut Workspaces) -> Result<Value, ApiError> {
    let params = serde_json::from_value::<DisconnectMixPortsParams>(params.clone())
        .map_err(|error| ApiError::new(ErrorCode::InvalidParams, error.to_string()))?;
    workspaces
        .disconnect_mix_ports(
