...
use kickhatsnare_protocol::{ErrorCode, workspace::DeleteTimelineTrackParams};
...
pub(super) fn handle(params: &Value, workspaces: &mut Workspaces) -> Result<Value, ApiError> {
    let params = serde_json::from_value::<DeleteTimelineTrackParams>(params.clone())
        .map_err(|error| ApiError::new(ErrorCode::InvalidParams, error.to_string()))?;
    workspaces
        .delete_timeline_track(&params.id)
