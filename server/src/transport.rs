...
mod tests {
    ...
    fn handles_a_ping_request() {
        let input = b"{\"protocolVersion\":1,\"id\":7,\"method\":\"system.ping\",\"params\":{}}\n";
        ...
        assert_eq!(
            ...
            "{\"protocolVersion\":1,\"id\":7,\"result\":\"ready\"}\n"
        );
    }
    ...
    fn routes_feature_domains_independently() {
        let input =
            b"{\"protocolVersion\":1,\"id\":8,\"method\":\"audio.unknown\",\"params\":{}}\n";
        let mut output = Vec::new();
        ...
        assert_eq!(
            String::from_utf8(output).expect("response should be UTF-8"),
            "{\"protocolVersion\":1,\"id\":8,\"error\":{\"code\":\"METHOD_NOT_FOUND\",\"message\":\"unknown audio method: unknown\"}}\n"
        );
    }

    #[test]
    fn rejects_a_mismatched_protocol_version() {
        let input = b"{\"protocolVersion\":2,\"id\":9,\"method\":\"system.ping\",\"params\":{}}\n";
        let mut output = Vec::new();
        ...
        assert_eq!(
            String::from_utf8(output).expect("response should be UTF-8"),
            "{\"protocolVersion\":1,\"id\":9,\"error\":{\"code\":\"PROTOCOL_VERSION_MISMATCH\",\"message\":\"expected protocol version 1, received 2\"}}\n"
        );
    }
}
